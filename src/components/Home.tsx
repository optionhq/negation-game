...
export default function Home() {
			...
			if (selectedPoint)
				...
				if (router.pathname.includes("/spaces/")) {
					...
					if (feedResult) {
						...
						if (!onlyReload) feedCursorRef.current = feedResult.cursor;
					}
				}
			}
	...
	const refreshItems = useCallback(
		async () => fetchItems({ onlyReload: true }),
		[
			points,
			setPoints,
			router,
	...
	useEffect(() => {
		...
		if (
			router.pathname.includes("spaces") &&
	...
	return (
		<main className="flex flex-col text-sm sm:text-base gap-4 my-8">
			{topic && (
				<h2 className="text-lg md:text-xl font-bold text-center mx-4 md:mx-12 p-4 border border-gray-300 bg-white rounded-xl sticky top-20 z-30 shadow-sm">
					{topic}
				</h2>
			)}
			{router.query.id && (
